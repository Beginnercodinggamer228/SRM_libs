...
namespace Mono.Cecil
{
	...
	internal sealed class MetadataBuilder
	{
		...
		{
			Guid mvid = CryptoService.ComputeGuid(CryptoService.ComputeHash(new ByteBuffer[]
			{
				this.data,
				this.resources,
				this.string_heap,
				this.user_string_heap,
				this.blob_heap,
				this.table_heap,
				this.code
			}));
			...
			this.guid_heap.WriteBytes(mvid.ToByteArray());
			this.guid_heap.position = position;
			this.module.Mvid = mvid;
		}
