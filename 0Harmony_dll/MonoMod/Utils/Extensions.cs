	...
	public static class Extensions
	{
		...
		public static Mono.Cecil.Cil.OpCode ToLongOp(this Mono.Cecil.Cil.OpCode op)
		{
			...
			lock (toLongOp)
			{
				...
				else
				{
					...
					result = (toLongOp2[code] = (((Mono.Cecil.Cil.OpCode?)((field != null) ? field.GetValue(null) : null)) ?? op));
				}
			}
			return result;
		}
		...
		public static Mono.Cecil.Cil.OpCode ToShortOp(this Mono.Cecil.Cil.OpCode op)
		{
			...
			lock (toShortOp)
			{
				...
				else
				{
					...
					result = (toShortOp2[code] = (((Mono.Cecil.Cil.OpCode?)((field != null) ? field.GetValue(null) : null)) ?? op));
				}
			}
			return result;
		}
		...
		public unsafe static void SetMonoCorlibInternal(this Assembly asm, bool value)
		{
			...
			lock (assemblyCache)
			{
				...
				ReflectionHelper.AssemblyCache[assemblyName.FullName] = value2;
				ReflectionHelper.AssemblyCache[assemblyName.Name] = value2;
			}
			long num = 0L;
		...
		public static Mono.Cecil.Cil.MethodBody Clone(this Mono.Cecil.Cil.MethodBody bo, MethodDefinition m)
		{
			...
			foreach (Instruction instruction in bc.Instructions)
			{
				...
				else
				{
					...
					if (array != null)
					{
						...
						{
							selector = (<>9__4 = ((Instruction i) => bc.Instructions[bo.Instructions.IndexOf(i)]));
						}
			...
			select new SequencePoint(bc.Instructions.FirstOrDefault((Instruction i) => i.Offset == o.Offset), o.Document)
			{
		...
		public static int GetManagedSize(this Type t)
		{
			...
			num = Extensions._GetManagedSizeHelper.MakeGenericMethod(new Type[]
			{
				t
