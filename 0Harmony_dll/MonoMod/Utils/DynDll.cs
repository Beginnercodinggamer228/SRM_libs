...
namespace MonoMod.Utils
{
	...
	internal static class DynDll
	{
		...
		private static void InternalResolveDynDllImports(Type type, object instance, Dictionary<string, List<DynDllMapping>> mappings)
		{
			...
			foreach (FieldInfo fieldInfo in type.GetFields(bindingFlags))
			{
				...
				while (j < customAttributes.Length)
				{
					...
					continue;
					IL_DC:
					foreach (string name in dynDllImportAttribute.EntryPoints.Concat(new string[]
					{
						fieldInfo.Name,
					...
					{
						IntPtr ptr;
						if (zero.TryGetFunction(name, out ptr))
						{
		...
		private static IntPtr dlopen(string filename, int flags)
		{
			...
			for (;;)
			{
				try
				{
					...
					if (num != 0 && num == 1)
					{
						result = DynDll.dl2_dlopen(filename, flags);
					}
					else
					{
						result = DynDll.dl_dlopen(filename, flags);
					}
				}
		...
		private static bool dlclose(IntPtr handle)
		{
			...
			for (;;)
			{
				try
				{
					...
					if (num != 0 && num == 1)
					{
						result = DynDll.dl2_dlclose(handle);
					}
					else
					{
						result = DynDll.dl_dlclose(handle);
					}
				}
				catch (DllNotFoundException obj) when (DynDll.dlVersion > 0)
				{
		...
		private static IntPtr dlsym(IntPtr handle, string symbol)
		{
			...
			for (;;)
			{
				try
				{
					...
					if (num != 0 && num == 1)
					{
						result = DynDll.dl2_dlsym(handle, symbol);
					}
					else
					{
						result = DynDll.dl_dlsym(handle, symbol);
					}
				}
				catch (DllNotFoundException obj) when (DynDll.dlVersion > 0)
				{
		...
		private static IntPtr dlerror()
		{
			...
			for (;;)
			{
				try
				{
					...
					if (num != 0 && num == 1)
					{
						result = DynDll.dl2_dlerror();
					}
					else
					{
						result = DynDll.dl_dlerror();
					}
				}
				catch (DllNotFoundException obj) when (DynDll.dlVersion > 0)
				{
		...
		{
			IntPtr result;
			if (!DynDll.InternalTryOpenLibrary(name, out result, skipMapping, flags))
			{
		...
		private static bool InternalTryOpenLibrary(string name, out IntPtr libraryPtr, bool skipMapping, int? flags)
		{
			...
			else
			{
				...
				libraryPtr = DynDll.dlopen(name, flags2);
				if (libraryPtr == IntPtr.Zero && File.Exists(name))
				{
					libraryPtr = DynDll.dlopen(Path.GetFullPath(name), flags2);
				}
			}
		...
		{
			IntPtr result;
			if (!DynDll.InternalTryGetFunction(libraryPtr, name, out result))
			{
