...
namespace MonoMod.Utils
{
	...
	internal static class _DMDEmit
	{
		...
		private static MethodBuilder _CreateMethodProxy(MethodBuilder context, MethodInfo target)
		{
			...
			string name = string.Format(".dmdproxy<{0}>?{1}", target.Name.Replace('.', '_'), target.GetHashCode());
		...
		public static void Generate(DynamicMethodDefinition dmd, MethodBase _mb, ILGenerator il)
		{
			...
			foreach (Instruction instruction3 in definition.Body.Instructions)
			{
				Label loc;
				...
				else
				{
					...
					if (array4 != null)
					{
						...
						instruction3.OpCode = instruction3.OpCode.ToLongOp();
					}
					else
					{
								...
								else
								{
									...
									else
									{
										...
										if (callSite != null)
										{
											if (dynamicMethod != null)
											{
												_DMDEmit._EmitCallSite(dynamicMethod, il, _DMDEmit._ReflOpCodes[instruction3.OpCode.Value], callSite);
												continue;
											}
					...
					if (mb != null)
					{
						...
						{
							if (!(instruction3.OpCode == Mono.Cecil.Cil.OpCodes.Call))
							{
								throw new NotSupportedException("Unsupported global method operand on opcode " + instruction3.OpCode.Name);
							}
							...
							else
							{
								...
								instruction3.OpCode = Mono.Cecil.Cil.OpCodes.Calli;
					...
					if (obj == null)
					{
						throw new NullReferenceException(string.Format("Unexpected null in {0} @ {1}", definition, instruction3));
					}
					il.DynEmit(_DMDEmit._ReflOpCodes[instruction3.OpCode.Value], obj);
				}
				if (!flag)
				{
		...
		internal static void _EmitCallSite(DynamicMethod dm, ILGenerator il, System.Reflection.Emit.OpCode opcode, Mono.Cecil.CallSite csite)
		{
			...
			_DMDEmit.<_EmitCallSite>g__AddArgument|17_4(clsArgument, requiredCustomModifiers, optionalCustomModifiers, ref CS$<>8__locals1);
			foreach (ParameterDefinition parameterDefinition in csite.Parameters)
			{
				...
				_DMDEmit.<_EmitCallSite>g__AddArgument|17_4(clsArgument2, requiredCustomModifiers2, optionalCustomModifiers2, ref CS$<>8__locals1);
			}
			...
			if (_DMDEmit._ILGen_emit_int != null)
			{
				...
				{
					_DMDEmit.<_EmitCallSite>g__GetTokenForSig|17_3(CS$<>8__locals1.signature, ref CS$<>8__locals1)
				});
				return;
			}
