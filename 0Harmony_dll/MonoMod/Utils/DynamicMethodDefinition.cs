...
namespace MonoMod.Utils
{
	...
	public sealed class DynamicMethodDefinition : IDisposable
	{
		...
		public DynamicMethodDefinition(MethodBase method) : this()
		{
			if (method == null)
			{
		...
		public DynamicMethodDefinition(string name, Type returnType, Type[] parameterTypes) : this()
		{
			this.Name = name;
		...
		public MethodInfo Generate(object context)
		{
			...
			string a = (environmentVariable != null) ? environmentVariable.ToLower(CultureInfo.InvariantCulture) : null;
			if (a == "dynamicmethod" || a == "dm")
			{
				return DMDGenerator<DMDEmitDynamicMethodGenerator>.Generate(this, context);
			}
			if (a == "methodbuilder" || a == "mb")
			{
