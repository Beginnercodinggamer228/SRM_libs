		...
		protected override MethodInfo _Generate(DynamicMethodDefinition dmd, object context)
		{
			...
			if (typeDefinition == null)
			{
				...
				else
				{
					...
					arg = ((name != null) ? name.Replace('.', '_') : null);
				}
				typeDefinition = new TypeDefinition(@namespace, string.Format(format, arg, this.GetHashCode()), Mono.Cecil.TypeAttributes.Public | Mono.Cecil.TypeAttributes.Abstract | Mono.Cecil.TypeAttributes.Sealed)
				{
			...
			try
			{
				...
				for (int i = 0; i < methodBody.Instructions.Count; i++)
				{
					...
					else
					{
						...
						{
							obj = metadataTokenProvider.Relink(relinker, clone);
						}
					}
					...
					{
						FieldReference fieldReference = obj as FieldReference;
				...
				{
					string text = Path.GetFullPath(Environment.GetEnvironmentVariable("MONOMOD_DMD_DUMP"));
					string path = module.Name + ".dll";
					string path2 = Path.Combine(text, path);
					text = Path.GetDirectoryName(path2);
