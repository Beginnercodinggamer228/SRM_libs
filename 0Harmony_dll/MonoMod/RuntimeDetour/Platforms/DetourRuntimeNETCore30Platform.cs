...
namespace MonoMod.RuntimeDetour.Platforms
{
	...
	public class DetourRuntimeNETCore30Platform : DetourRuntimeNETCorePlatform
	{
		...
		{
			IntPtr from = DetourHelper.Native.MemAlloc(64U);
			NativeDetourData nativeDetourData = DetourHelper.Native.Create(from, target, null);
		...
		private unsafe DetourRuntimeNETCore30Platform.CorJitResult CompileMethodHook(IntPtr jit, IntPtr corJitInfo, in DetourRuntimeNETCore30Platform.CORINFO_METHOD_INFO methodInfo, uint flags, out byte* nativeEntry, out ulong nativeSizeOfCode)
		{
			...
			DetourRuntimeNETCore30Platform.CorJitResult result;
			try
			{
				...
				if (DetourRuntimeNETCore30Platform.hookEntrancy == 1)
				{
					try
					{
						...
						RuntimeMethodHandle methodHandle = this.CreateHandleForHandlePointer(methodInfo.ftn);
						this.JitHookCore(typeHandle, methodHandle, (IntPtr)nativeEntry, nativeSizeOfCode, array, array2);
					}
					catch
					{
					}
		...
		protected virtual void SetupJitHookHelpers()
		{
			...
			this.MethodHandle_GetLoaderAllocator = method2.CreateDelegate<DetourRuntimeNETCore30Platform.d_MethodHandle_GetLoaderAllocator>();
			...
			this.GetDeclaringTypeOfMethodHandle = method4.CreateDelegate<DetourRuntimeNETCore30Platform.d_GetDeclaringTypeOfMethodHandle>();
			Type[] array = new Type[]
			{
				typeof(IntPtr),
				typeof(object)
			...
			this.CreateRuntimeMethodInfoStub = method5.CreateDelegate<DetourRuntimeNETCore30Platform.d_CreateRuntimeMethodInfoStub>();
			...
			MethodInfo method6;
			using (DynamicMethodDefinition dynamicMethodDefinition4 = new DynamicMethodDefinition("new RuntimeMethodHandle", typeof(RuntimeMethodHandle), new Type[]
			{
				typeof(object)
			}))
			{
			...
			this.CreateRuntimeMethodHandle = method6.CreateDelegate<DetourRuntimeNETCore30Platform.d_CreateRuntimeMethodHandle>();
		}
		...
		{
			IntPtr value = (IntPtr)DetourRuntimeNETCore30Platform._runtimeAssemblyPtrField.GetValue(assembly);
		...
		{
			NativeDetourData detour = DetourHelper.Native.Create(from, to, null);
