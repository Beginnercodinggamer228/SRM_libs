...
namespace MonoMod.RuntimeDetour.Platforms
{
	...
	public abstract class DetourRuntimeILPlatform : IDetourRuntimePlatform
	{
		...
		public unsafe DetourRuntimeILPlatform()
		{
			...
			{
				DetourRuntimeILPlatform.GlueThiscallStructRetPtrOrder* value = ptr;
				((Func<IntPtr, IntPtr, IntPtr, DetourRuntimeILPlatform._SelftestStruct>)Delegate.CreateDelegate(typeof(Func<IntPtr, IntPtr, IntPtr, DetourRuntimeILPlatform._SelftestStruct>), this, method3))((IntPtr)((void*)value), (IntPtr)((void*)value), arg);
			}
			...
			fixed (DetourRuntimeILPlatform.GlueThiscallStructRetPtrOrder* ptr = &this.GlueThiscallInStructRetPtr)
			{
				...
				object firstArgument = default(DetourRuntimeILPlatform._SelftestStruct);
				*ptr2 = (DetourRuntimeILPlatform.GlueThiscallStructRetPtrOrder)((Func<short>)Delegate.CreateDelegate(typeof(Func<short>), firstArgument, method5))();
				if (*ptr2 == (DetourRuntimeILPlatform.GlueThiscallStructRetPtrOrder)(-1))
				{
			...
			if (DynamicMethodDefinition.IsDynamicILAvailable)
			{
				...
				using (DynamicMethodDefinition dynamicMethodDefinition = new DynamicMethodDefinition(DetourRuntimeILPlatform._MemAllocScratchDummy))
				{
					...
					method7 = DMDGenerator<DMDEmitDynamicMethodGenerator>.Generate(dynamicMethodDefinition, null);
				}
				this.Pin(method7);
				this.ReferenceDynamicPoolPtr = this.GetNativeStart(method7);
			}
		}
		...
		private void _HookSelftest(MethodInfo from, MethodInfo to)
		{
			this.Pin(from);
			this.Pin(to);
			NativeDetourData detour = DetourHelper.Native.Create(this.GetNativeStart(from), this.GetNativeStart(to), null);
		...
		public uint TryMemAllocScratchCloseTo(IntPtr target, out IntPtr ptr, int size)
		{
			...
			using (DynamicMethodDefinition dynamicMethodDefinition = new DynamicMethodDefinition(DetourRuntimeILPlatform._MemAllocScratchDummy))
			{
				...
				if (flag2)
				{
					method = DMDGenerator<DMDEmitDynamicMethodGenerator>.Generate(dynamicMethodDefinition, null);
				}
				else
				{
					method = DMDGenerator<DMDCecilGenerator>.Generate(dynamicMethodDefinition, null);
				}
			}
			this.Pin(method);
			ptr = this.GetNativeStart(method);
