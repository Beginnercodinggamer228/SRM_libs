...
namespace MonoMod.RuntimeDetour.Platforms
{
	...
	public class DetourRuntimeNETCorePlatform : DetourRuntimeNETPlatform
	{
		...
		{
			int index = DetourRuntimeNETCorePlatform.isNet5Jit ? 2 : 4;
			Guid result;
			DetourRuntimeNETCorePlatform.ReadObjectVTable(jit, index).AsDelegate<DetourRuntimeNETCorePlatform.d_getVersionIdentifier>()(jit, out result);
			return result;
		}
		...
		protected virtual void JitHookCore(RuntimeTypeHandle declaringType, RuntimeMethodHandle methodHandle, IntPtr methodBodyStart, ulong methodBodySize, RuntimeTypeHandle[] genericClassArguments, RuntimeTypeHandle[] genericMethodArguments)
		{
			try
			{
				...
				{
					MMDbgLog.Log(string.Format("Error executing OnMethodCompiled event: {0}", arg));
				}
			}
			catch (Exception arg2)
			{
				MMDbgLog.Log(string.Format("Error in JitHookCore: {0}", arg2));
			}
		}
		...
		public static DetourRuntimeNETCorePlatform Create()
		{
			...
			catch (Exception arg)
			{
				...
				MMDbgLog.Log(string.Format("Error: {0}", arg));
			}
			return new DetourRuntimeNETCorePlatform();
		}
