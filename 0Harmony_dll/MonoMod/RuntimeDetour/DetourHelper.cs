	...
	internal static class DetourHelper
	{
		...
		public static IDetourRuntimePlatform Runtime
		{
			get
			{
				...
				IDetourRuntimePlatform result;
				lock (runtimeLock)
				{
					if (DetourHelper._Runtime != null)
					{
						result = DetourHelper._Runtime;
					}
		...
		public static IDetourNativePlatform Native
		{
			get
			{
				...
				IDetourNativePlatform result;
				lock (nativeLock)
				{
					if (DetourHelper._Native != null)
					{
						result = DetourHelper._Native;
					}
					...
					else
					{
						...
						{
							detourNativePlatform = new DetourNativeARMPlatform();
						}
						else
						{
							detourNativePlatform = new DetourNativeX86Platform();
						}
						if (PlatformHelper.Is(Platform.Windows))
						{
							result = (DetourHelper._Native = new DetourNativeWindowsPlatform(detourNativePlatform));
						}
						else
						{
							if (ReflectionHelper.IsMono)
							{
								try
								{
									return DetourHelper._Native = new DetourNativeMonoPlatform(detourNativePlatform, "libmonosgen-2.0." + PlatformHelper.LibrarySuffix);
								}
								catch
								{
								}
							...
							if ((ReflectionHelper.IsMono && environmentVariable != "0") || environmentVariable == "1")
							{
								try
								{
									return DetourHelper._Native = new DetourNativeMonoPosixPlatform(detourNativePlatform);
								}
								catch
								{
								}
							...
							try
							{
								return DetourHelper._Native = new DetourNativeLibcPlatform(detourNativePlatform);
							}
							catch
							{
							}
							result = detourNativePlatform;
						}
					}
				}
				return result;
		...
		public static MethodInfo GenerateNativeProxy(IntPtr target, MethodBase signature)
		{
			...
			Type returnType = ((methodInfo != null) ? methodInfo.ReturnType : null) ?? typeof(void);
			...
			using (DynamicMethodDefinition dynamicMethodDefinition = new DynamicMethodDefinition("Native<" + ((long)target).ToString("X16", CultureInfo.InvariantCulture) + ">", returnType, array))
			{
				methodInfo2 = dynamicMethodDefinition.StubCriticalDetour().Generate().Pin<MethodInfo>();
			}
			NativeDetourData detour = DetourHelper.Native.Create(methodInfo2.GetNativeStart(), target, null);
		...
		private static readonly ConstructorInfo _ctor_Exception = typeof(Exception).GetConstructor(new Type[]
		{
			typeof(string)
		});
	}
